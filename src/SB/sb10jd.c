    ...
    i32* info)
{
    ...
    *info = 0;
    ...
    if (ns1 > 0) {
        ...
        SLC_DLACPY("Full", &np, &n, c, &ldc, &dwork[iwrk], &np);
        SLC_DGEMM("N", "T", &np, &n, &n, &one, &dwork[iwrk], &np,
        ...
        if (k > 0) {
            ...
            SLC_DGEMM("N", "N", &np, &ns1, &k, &mone, &dwork[ic2], &np,
                      &dwork[ia21], &k, &one, c, &ldc);

            SLC_DGEMM("N", "N", &np, &m, &k, &mone, &dwork[ic2], &np,
