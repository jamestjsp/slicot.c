    ...
    if (n1 > 0 && m1 > 0) {
        if (lover) {
            if (n1 * m1 <= (*n) * n2) {
                ...
                if (m1 <= (*n) * n2) {
                    for (i32 j = 0; j < n1; j++) {
                        SLC_DCOPY(&m1, &b[j], &ldb, &a[n1 * lda], &ONE);
                        i32 one = 1;
                        SLC_DGEMV("T", &m1, &m1, &ONE, dwork, &ldwm1,
                                  &a[n1 * lda], &one, &ZERO, &b[j], &ldb);
                    }
                } else {
                    for (i32 j = 0; j < n1; j++) {
                        SLC_DCOPY(&m1, &b[j], &ldb, &dwork[m1 * m1], &ONE);
                        i32 one = 1;
                        SLC_DGEMV("T", &m1, &m1, &ONE, dwork, &ldwm1,
                                  &dwork[m1 * m1], &one, &ZERO, &b[j], &ldb);
                    }
                }
            } else if (m1 <= (*n) * n2) {
                for (i32 j = 0; j < n1; j++) {
                    SLC_DCOPY(&m1, &b1[j], &ldb1, &a[n1 * lda], &ONE);
                    i32 one = 1;
                    SLC_DGEMV("T", &m1, &m1, &ONE, dwork, &ldwm1,
                              &a[n1 * lda], &one, &ZERO, &b[j], &ldb);
                }
                ...
                for (i32 j = 0; j < n1; j++) {
                    SLC_DCOPY(&m1, &b1[j], &ldb1, &dwork[m1 * m1], &ONE);
                    i32 one = 1;
                    SLC_DGEMV("T", &m1, &m1, &ONE, dwork, &ldwm1,
                              &dwork[m1 * m1], &one, &ZERO, &b[j], &ldb);
                }
