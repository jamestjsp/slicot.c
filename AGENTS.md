...
- Setup (first time): `./scripts/setup_venv.sh && source venv/bin/activate`
...
- **Types**: `INTEGER` → `i32`, `DOUBLE PRECISION` → `f64`, `LOGICAL` → `bool`
...
- **Error Codes**: `info = 0` success, `info < 0` param error, `info > 0` algorithm error

**Translation Workflow:**
- **Use Droid**: Use the `slicot-fortran-translator` droid (defined in `.factory/droids/`) for the full TDD workflow (RED/GREEN/REFACTOR/VERIFY).
